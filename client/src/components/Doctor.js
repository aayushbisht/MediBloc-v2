...
const Doctor = ({ ipfs, mediBloc, account }) => {
  ...
  const getDoctorData = async () => {
    var doctor = await mediBloc.methods.doctorInfo(account).call();
    setDoctor(doctor);
  };
  const getPatientAccessList = async () => {
    var pat = await mediBloc.methods.getDoctorPatientList(account).call();
    let pt = [];
  ...
  const getTransactionsList = async () => {
    ...
    for (let i = transactionsIdList.length - 1; i >= 0; i--) {
      let transaction = await mediBloc.methods
        ...
        .call();
      let sender = await mediBloc.methods
        .patientInfo(transaction.sender)
        .call();
      if (!sender.exists)
        sender = await mediBloc.methods.insurerInfo(transaction.sender).call();
      transaction = {
        ...transaction,
