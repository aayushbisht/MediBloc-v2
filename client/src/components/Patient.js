...
const Patient = ({ mediBloc, account, ethValue }) => {
  ...
  const getPatientData = async () => {
    var patient = await mediBloc.methods.patientInfo(account).call();
    setPatient(patient);
  };
  const giveAccess = (e) => {
    e.preventDefault();
  ...
  const revokeAccess = async (email) => {
    var addr = await mediBloc.methods.emailToAddress(email).call();
    mediBloc.methods
      .revokeAccess(addr)
      .send({ from: account })
      .on("transactionHash", (hash) => {
  ...
  const getDoctorAccessList = async () => {
    var doc = await mediBloc.methods.getPatientDoctorList(account).call();
    let dt = [];
    for (let i = 0; i < doc.length; i++) {
      let doctor = await mediBloc.methods.doctorInfo(doc[i]).call();
      dt = [...dt, doctor];
    }
    setDocList(dt);
  };
  const getInsurer = async () => {
    var insurer = await mediBloc.methods
      .insurerInfo(patient.policy.insurer)
      .call();
    setInsurer(insurer);
  };
  const getInsurerList = async () => {
    var ins = await mediBloc.methods.getAllInsurersAddress().call();
    let it = [];
    for (let i = 0; i < ins.length; i++) {
      let insurer = await mediBloc.methods.insurerInfo(ins[i]).call();
  ...
  const getPolicyList = async () => {
    var policyList = await mediBloc.methods
  ...
  const getTransactionsList = async () => {
    var transactionsIdList = await mediBloc.methods
      .getPatientTransactions(account)
      .call();
    let tr = [];
    for (let i = transactionsIdList.length - 1; i >= 0; i--) {
      let transaction = await mediBloc.methods
        .transactions(transactionsIdList[i])
        .call();
      let doctor = await mediBloc.methods
