...
const Insurer = ({ mediBloc, account, ethValue }) => {
  ...
  const getInsurerData = async () => {
    var insurer = await mediBloc.methods.insurerInfo(account).call();
    setInsurer(insurer);
  };
  const getPolicyList = async () => {
    var pol = await mediBloc.methods.getInsurerPolicyList(account).call();
    setPolicyList(pol);
  };
  const createPolicy = (e) => {
    e.preventDefault();
    mediBloc.methods
      .createPolicy(polName, polCoverValue, polDuration, polPremium)
      .send({ from: account })
  ...
  const handleShowRecordModal = async (e, patient) => {
    var record = {};
    await fetch(
      `${process.env.REACT_APP_INFURA_DEDICATED_GATEWAY}/${patient.record}`
    )
      .then((res) => res.json())
  ...
  const getPatientList = async () => {
    var pat = await mediBloc.methods.getInsurerPatientList(account).call();
    let pt = [];
    for (let i = 0; i < pat.length; i++) {
      let patient = await mediBloc.methods.patientInfo(pat[i]).call();
      pt = [...pt, patient];
    }
    setPatList(pt);
  };
  const getClaimsData = async () => {
    var claimsIdList = await mediBloc.methods.getInsurerClaims(account).call();
    let cl = [];
    for (let i = claimsIdList.length - 1; i >= 0; i--) {
      let claim = await mediBloc.methods.claims(claimsIdList[i]).call();
      let patient = await mediBloc.methods.patientInfo(claim.patient).call();
      let doctor = await mediBloc.methods.doctorInfo(claim.doctor).call();
      claim = {
        ...claim,
        id: claimsIdList[i],
  ...
  const approveClaim = async (e, claim) => {
    ...
    mediBloc.methods
      .approveClaimsByInsurer(claim.id)
      .send({
        from: account,
  ...
  const rejectClaim = async (e, claim) => {
    mediBloc.methods
      .rejectClaimsByInsurer(claim.id)
      .send({ from: account })
      .on("transactionHash", (hash) => {
        return (window.location.href = "/login");
      });
            ...
            <Form>
              ...
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  type="number"
                  value={polCoverValue}
                  onChange={(e) => setPolCoverValue(e.target.value)}
                  placeholder="Enter the cover value in INR"
                ></Form.Control>
              </Form.Group>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  placeholder="Enter the annual premium in INR"
                ></Form.Control>
              </Form.Group>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  min={1}
                  value={polDuration}
                  onChange={(e) => setPolDuration(e.target.value)}
                  ...
                  policyList.map((pol, idx) => {
                    return (
                      <tr key={idx + 1}>
                        ...
                        <td>
                          {pol.timePeriod} Year{pol.timePeriod > 1 ? "s" : ""}
                        </td>
                      </tr>
                    );
                  })
                    ...
                    return (
                      <tr key={idx + 1}>
                        ...
                        <td>
                          <Button
                            variant="coolColor"
                            onClick={(e) => handleShowRecordModal(e, pat)}
                          >
                            View
                          </Button>
                        </td>
                            ...
                            <tr key={idx + 1}>
                              ...
                              <td>
                                {treatment.prescription ? (
                                  <Link
                                    to={`${process.env.REACT_APP_INFURA_DEDICATED_GATEWAY}/${treatment.prescription}`}
                                    target="_blank"
                                  >
